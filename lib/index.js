...
void function () {
  var fs, getGlobPath, glob, globber, isFile, isGlob, partitionOptions, Path, supportedOptions;
  fs = require('fs');
  glob = require('glob');
  Path = require('path');
  supportedOptions = [
  ...
  partitionOptions = function (options) {
    var cache$, key, other, result, supported, value;
    cache$ = result = [
      {},
      {}
    ];
    ...
    for (key in options) {
      ...
      value = options[key];
      (function (key, value) {
        return (in$(key, supportedOptions) ? supported : other)[key] = value;
      }(key, value));
    }
    return [
      supported,
